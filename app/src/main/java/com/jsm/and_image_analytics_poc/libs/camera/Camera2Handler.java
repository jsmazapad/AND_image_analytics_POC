...
import static com.jsm.and_image_analytics_poc.config.ConfigConstants.FRAGMENT_DIALOG;
...
public class Camera2Handler {
     ...
     * Fine tune this parameter to avoid camera bus limitations
     */
        ...
        public void onError(@NonNull CameraDevice cameraDevice, int error) {
            ...
            switch (error){
                case ERROR_CAMERA_DEVICE:
                    errorMessage = "ERROR_CAMERA_DEVICE";
                    initCamera(mTextureView);

                    break;
                case ERROR_CAMERA_DISABLED:
                    errorMessage = "ERROR_CAMERA_DISABLED";
                    break;

                case ERROR_CAMERA_IN_USE:
                    errorMessage = "ERROR_CAMERA_IN_USE";
                    break;

                case ERROR_CAMERA_SERVICE:
                    errorMessage = "ERROR_CAMERA_SERVICE";
                    break;
                case ERROR_MAX_CAMERAS_IN_USE:
                    errorMessage = "ERROR_MAX_CAMERAS_IN_USE";
                    break;
            }

            Log.e("CAMERA DEVICE ERROR", errorMessage);
...
//            }
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            mBackgroundHandler.post(new Camera2Handler.ImageSaver(reader.acquireNextImage(), mFile));
        }
    ...
    public Camera2Handler(AutoFitTextureView mTextureView, Fragment fragment, ImageReceivedCallback imageReceivedCallback) {
    ...
    private static Size chooseOptimalSize(Size[] choices, int textureViewWidth,
        ...
        if (bigEnough.size() > 0) {
            return Collections.min(bigEnough, new Camera2Handler.CompareSizesByArea());
        } else if (notBigEnough.size() > 0) {
            return Collections.max(notBigEnough, new Camera2Handler.CompareSizesByArea());
        } else {
    ...
    public void initCamera(TextureView mTextureView){
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                //TODO Seleccionar output size
                // For still image captures, we use the largest available size.
                Size largest = Collections.max(
                        Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)),
                        new Camera2Handler.CompareSizesByArea());
            ...
            //TODO handle error
            Camera2BasicFragment.ErrorDialog.newInstance(fragment.getActivity().getString(R.string.camera_error))
                    .show(fragment.getChildFragmentManager(), ConfigConstants.FRAGMENT_DIALOG);
        }
    }
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                    ...
                    imageReceivedCallback.onImageReceived(mFile);
                    //TODO Borrar código
                    unlockFocus();
                }
            };
    ...
    /**
     * REquest permissions, only needed in  API > M
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    private void requestCameraPermission() {
        if (fragment.getActivity().shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
            new Camera2BasicFragment.ConfirmationDialog().show(fragment.getChildFragmentManager(), FRAGMENT_DIALOG);
        } else {
            fragment.getActivity().requestPermissions(new String[]{Manifest.permission.CAMERA}, ConfigConstants.REQUEST_CAMERA_PERMISSION);
        }
    }
}
